...
class Evaluator(object):
    ...
    def __init__(self, output_pcl_dir, dataset_root, dataset, summary_dir, experiment_name, device='cuda', res_gts='8192_poisson', logger=BlackHole()):
        ...
        # self.gts_pcl_dir = os.path.join(dataset_root, dataset, res_gts)
        # self.gts_mesh_dir = os.path.join(dataset_root, dataset, 'original_meshes')
        ### PU-GAN
        ...
        ### sketchfab
        # self.gts_pcl_dir = os.path.join(dataset_root, dataset, 'point_cloud', 'test', res_gts)
        # self.gts_mesh_dir = os.path.join(dataset_root, dataset, 'mesh', 'test_mesh')
    ...
    def run(self):
        ...
        for name in tqdm(pcls_name, desc='Evaluate'):
            ...
            pcl_up = pcls_up[name].unsqueeze(0).to(self.device)
            if pcl_up.size(-1) == 6:
                pcl_up = pcl_up[:, :, :3]
        ...
        update_summary(
            ...
            metrics={
                ...
                'cd_sph(std)': res_std['cd_sph'],
                'p2f(mean)': res_mean['p2f'],
                'p2f(std)': res_std['p2f'],
                'hd_sph(mean)': res_mean['hd_sph'],
                'hd_sph(std)': res_std['hd_sph'],
            }
        )
...
if __name__=='__main__':
    output_pcl_dir = 'PU-GCN-test/baseline-k-20'
    dataset_root = 'PU-GCN-test'
